...
class TextToSQLAgent:
    ...
    def _build_system_prompt(self) -> str:
        ...
        return (
            ...
            "2. Always follow this exact sequence for every query:\n"
            "   a. Call submit_sql_query with your generated SQL.\n"
            "   b. Call execute_sql with the SQL from step (a) to run it against the database.\n"
            "   c. Call format_results with the rows returned by execute_sql.\n"
            "   Never skip execute_sql — do not return SQL text as the answer.\n"
            "3. Call validate_sql only for complex or uncertain queries, between steps (a) and (b).\n"
            "4. If execute_sql returns an error, call recover_from_error immediately.\n"
            "5. Only generate SELECT queries unless the user explicitly requests writes "
            "   AND write mode is enabled.\n"
            "6. Use recall_context to reference previous queries in multi-turn conversations.\n"
            "7. Be concise. Answer directly — do not over-explain.\n\n"
            f"{schema_text}"
        )
